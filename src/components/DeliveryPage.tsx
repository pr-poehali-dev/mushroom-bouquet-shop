    ...
    <div>
      ...
      <section className="hero-gradient py-20">
        <div className="container mx-auto px-4">
          <p className="font-body text-sm text-[hsl(355,65%,55%)] uppercase tracking-widest mb-4">Логистика</p>
          <h1 className="font-display text-5xl md:text-6xl font-light text-foreground mb-6">
            Доставка<br />
            <em className="text-[hsl(355,65%,55%)] not-italic">по Калининграду</em>
          </h1>
      ...
      <section className="py-20 container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative">
          <div className="hidden md:block absolute top-8 left-1/4 right-1/4 h-px bg-[hsl(350,40%,88%)] z-0" />
          {[
            ...
            <div key={step} className="relative z-10 bg-white border border-[hsl(350,30%,92%)] rounded-2xl p-6 text-center hover-lift">
              <div className="font-display text-4xl text-[hsl(350,40%,88%)] mb-3">{step}</div>
              <div className="w-12 h-12 bg-[hsl(350,50%,93%)] rounded-xl flex items-center justify-center mx-auto mb-4">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-2xl mx-auto space-y-3">
            {ZONES.map(({ zone, time, price }) => (
              <div key={zone} className="bg-white rounded-2xl p-5 flex items-center justify-between border border-[hsl(350,30%,92%)] hover-lift">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-[hsl(350,50%,93%)] rounded-xl flex items-center justify-center">
                    <Icon name="MapPin" size={18} className="text-[hsl(355,65%,55%)]" />
                  </div>
                  <div>
                ...
                <div className="font-body font-semibold text-[hsl(355,65%,50%)]">{price}</div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {[
            ...
            <div key={title} className="bg-white border border-[hsl(350,30%,92%)] rounded-2xl p-6 hover-lift">
              <div className="w-12 h-12 bg-[hsl(350,50%,93%)] rounded-xl flex items-center justify-center mb-4">
                <Icon name={icon} fallback="Star" size={20} className="text-[hsl(355,65%,55%)]" />
              </div>
              <h3 className="font-display text-xl font-light text-foreground mb-2">{title}</h3>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-3">
            {FAQ.map(({ q, a }) => (
              <details key={q} className="group bg-white border border-[hsl(350,30%,92%)] rounded-2xl overflow-hidden">
                <summary className="flex items-center justify-between p-5 cursor-pointer list-none font-body font-medium text-foreground hover:text-[hsl(355,65%,50%)] transition-colors">
                  {q}
                  <Icon name="ChevronDown" size={18} className="text-foreground/40 group-open:rotate-180 transition-transform" />
                </summary>
                <div className="px-5 pb-5 font-body text-sm text-foreground/60 leading-relaxed border-t border-[hsl(350,30%,92%)] pt-4">
                  {a}
                </div>
              </details>
            ))}
