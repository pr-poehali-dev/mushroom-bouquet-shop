...
export default function CartPage({ cart, onRemove, onUpdateQty }: CartPageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <p className="font-body text-sm text-[hsl(130,35%,30%)] uppercase tracking-widest mb-2">Покупки</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cart.map(item => (
            <div key={item.id} className="bg-white border border-[hsl(100,15%,86%)] rounded-2xl p-5 flex gap-5 items-center animate-slide-up">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-body text-sm text-[hsl(4,72%,50%)] font-medium">
                  {item.price.toLocaleString('ru-RU')} ₽ / шт
                </p>
              </div>

              {/* Qty */}
              <div className="flex items-center gap-2 bg-[hsl(120,15%,93%)] rounded-full px-2 py-1">
                <button
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-[hsl(100,15%,86%)] rounded-3xl p-6 sticky top-24">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between font-body text-sm text-foreground/60">
                <span>Доставка</span>
                <span className="text-[hsl(130,40%,35%)] font-medium">Рассчитывается</span>
              </div>
              <div className="border-t border-[hsl(100,15%,86%)] pt-3 flex justify-between">
                <span className="font-body font-semibold text-foreground">Итого</span>
                <span className="font-body font-bold text-[hsl(4,72%,48%)] text-xl">
                  {total.toLocaleString('ru-RU')} ₽
                </span>
              </div>
            </div>

            <button className="w-full bg-[hsl(4,72%,50%)] text-white font-body font-medium py-4 rounded-full hover:bg-[hsl(4,72%,43%)] transition-all duration-300 hover:-translate-y-0.5 shadow-lg shadow-[hsl(4,72%,50%)]/20 mb-3">
              Оформить заказ
            </button>
            ...
            <div className="mt-6 pt-6 border-t border-[hsl(100,15%,86%)]">
              <div className="flex items-start gap-3">
                <Icon name="Truck" size={18} className="text-[hsl(130,35%,32%)] flex-shrink-0 mt-0.5" />
                <div>
